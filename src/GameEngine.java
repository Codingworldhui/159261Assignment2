...
public abstract class GameEngine implements KeyListener, MouseListener, MouseMotionListener{
    ...
    public void setupWindow(int width, int height) {
        ...
        JPanel mInitialPanel = new JPanel();
        ImageIcon icon1 = new ImageIcon("src/button.jpg");
        ImageIcon icon2 = new ImageIcon("src/initbackground.jpg");
        // 设置按钮的大小
        Dimension buttonSize = new Dimension(width/6, height/11);
        JLabel initialLabel = new JLabel("Initial Page",icon2,JLabel.CENTER);
        JButton startButton = new JButton("Start",icon1);
...
//                mFrame.add()
        ...
        initialLabel.setLayout(new GridBagLayout());
...
// 将组件放置在网格的中心位置
        ...
        initialLabel.add(startButton, gbc);

        mInitialPanel.setLayout(new BorderLayout());
        mInitialPanel.add(initialLabel, BorderLayout.CENTER);
        mInitialPanel.setLayout(new BorderLayout());
        mInitialPanel.add(initialLabel, BorderLayout.CENTER);
//        mInitialPanel.add(startButton, BorderLayout.SOUTH);
        mWidth = width;
        mHeight = height;
