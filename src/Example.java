        ...
        for (Monster monster : monsters) {
            if (!monster.is_appear) {
                if (distance(littlehero.HeroPositionX, littlehero.HeroPositionY, monster.monsterPositionX, monster.monsterPositionY) > 100) {
                    ...
                    if((distance(littlehero.HeroPositionX, littlehero.HeroPositionY, monster.monsterPositionX, monster.monsterPositionY) < 200)&& littlehero.is_attacking ){
                        playAudio(audioClip);
                        monster.is_appear = true;
                        score++;
                        monster.setDefeated(true);
                    }
                    if (distance(littlehero.HeroPositionX, littlehero.HeroPositionY, monster.monsterPositionX, monster.monsterPositionY) <= 10) {
                        healthCount--;
                        littlehero.be_attacked = true;
//                        playAudio(audioClip);
                        playAudio(heroHurtSound);
                        monster.is_appear = true;
...
//                        score++;
    ...
    public void update(double dt) {
        ...
        if(distance(littlehero.HeroPositionX,littlehero.HeroPositionY,800,340 )< 100 && !exampleCreated) {
        ...
        if (direction.equals("right")) {
            switch (attackMode) {
                case 1:
                    ...
                    if (currentFrame_attack>7){
                        littlehero.is_attacking=false;
                    }
                    break;
                case 2:
...
//                    littlehero.is_attacking=false;
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (e.getKeyCode() == KeyEvent.VK_J) {
            littlehero.is_attacking=true;
            playAudio(heroAttackSound);
            attackMode = 1;
            rest = 0;
        }
